...
export default defineComponent({
  ...
  mounted() {},
  methods: {
    // แก้ method closeDialog ให้ส่ง id กลับ
    closeDialog(reload: boolean = false, newOrgId: string | null = null) {
      this.CloseDialogOrganiz(false, reload, newOrgId)
    },
    async CreateOrganization() {
      ...
      if (valid) {
        try {
          ...
          if (response) {
            const newOrgId = response // << สำคัญ: ดึง id ที่สร้างใหม่
            setTimeout(() => {
              this.loading = false
              this.sweetAlertStore.success('เพิ่มข้อมูลหน่วยงานสำเร็จ!')
              this.closeDialog(true, newOrgId) // << ส่ง id กลับไป
            }, 600)
          }
        } catch (error) {
          console.error(error)
