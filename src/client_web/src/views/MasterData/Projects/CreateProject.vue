  ...
  <v-row class="d-flex align-center justify-center">
    ...
    >
      <v-card class="page-container scroll-content dialog-scrollbar">
        <v-card-text>
          <span class="text-sub-title">เพิ่มข้อมูลโครงการ</span>
          <v-form
            ref="form"
          ...
          >
            <v-row class="px-1 px-sm-3 px-md-5 mt-3">
              ...
              <v-col
                ...
                md="4"
              >
                <label class="form-label">รหัสโครงการ <span class="text-error">*</span></label>
                <v-text-field
                  v-model="createCommand.projectCode"
              ...
              >
                ...
                <v-text-field
                  v-model="createCommand.contractNumber"
              ...
              <v-col
                ...
                md="4"
              >
                <label>ชื่อย่อโครงการ <span class="text-error">*</span></label>
                <v-text-field
                  v-model="createCommand.shortName"
                  placeholder="ระบุชื่อย่อโครงการ"
                  variant="outlined"
                  density="comfortable"
                  color="primary-darken-0"
                  :rules="projectShortNameRules"
                ></v-text-field>
              </v-col>
              <!-- ชื่อโครงการ -->
              <v-col
                ...
                md="12"
              >
                <label class="form-label">ชื่อโครงการ <span class="text-error">*</span></label>
                <v-text-field
                  v-model="createCommand.projectName"
              ...
              <v-col
                ...
                md="6"
              >
                <label class="form-label">วันที่สิ้นสุดประกัน</label>
                <div>
                  <TextFieldDatepicker
              ...
              <v-col
                ...
                md="6"
              >
                <label class="form-label">ประเภทโครงการ <span class="text-error">*</span></label>
                <v-autocomplete
                  v-model="createCommand.projectType"
              ...
              <v-col
                ...
                md="6"
              >
                <label class="form-label">มูลค่าโครงการ</label>
                <v-text-field
...
import { Client, CreateProjectCommand } from '@/client'
...
export default defineComponent({
  ...
  watch: {
    'createCommand.projectCost'(newVal: any) {
  ...
  methods: {
    ...
    async CreateOrganization() {
      ...
      if (valid) {
        try {
          this.loading = true
          const response = await client.createProject(this.createCommand)
          console.log('Create project response:', response) // Debug log
          if (response) {
            setTimeout(() => {
              this.loading = false
              this.sweetAlertStore.success('เพิ่มข้อมูลโครงงานสำเร็จ!')
              // ส่งข้อมูลที่ใช้สร้างไปด้วย เพื่อหา ID ที่ตรงกัน
              const searchData = {
                projectCode: this.createCommand.projectCode,
                projectName: this.createCommand.projectName,
                contractNumber: this.createCommand.contractNumber,
                organizationId: this.createCommand.organizationId,
              }
              console.log('Sending project search data:', searchData) // Debug log
    ...
    changeTimewarrantyEndDate(value: any) {
      this.createCommand.warrantyEndDate = value
    },
    changeTimeSignedDate(value: any) {
      this.createCommand.contractSignedDate = value
    },
    ...
    onInputCost(event: any) {
      ...
      if (!isNaN(rawValue)) {
        this.createCommand.projectCost = parseFloat(rawValue)
        this.formattedCost = rawValue
      }
    },
    formatCost() {
      this.formattedCost = this.formatNumber(this.createCommand.projectCost)
    },
  },
})
</script>
