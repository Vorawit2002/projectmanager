    ...
    >
      <v-card class="page-container scroll-content dialog-scrollbar">
        <v-card-text>
          ...
          <v-form
            ref="form"
            @submit.prevent="UpdateProjectContact"
            class="text-black"
          >
...
<script lang="ts">
import { Client, GetOrganizationContactByOrganizationIdQuery, UpdateProjectContactCommand } from '@/client'
...
export default defineComponent({
  ...
  watch: {
    ...
    'UpdateCommand.projectId'(newProjectId) {
      ...
      if (selectedProject && selectedProject.organizationId) {
        ...
        this.UpdateCommand.organizationContactId = undefined
      } else {
  ...
  methods: {
    async initialize() {
      try {
        ...
        console.log('ProjectContact response:', response)
        if (response) {
          this.UpdateCommand = response as UpdateProjectContactCommand
          console.log('UpdateCommand after load:', this.UpdateCommand)
          if (this.UpdateCommand.projectId) {
            const project = await client.getProjectQueryByID(this.UpdateCommand.projectId as any)
            console.log('Project details:', project)
            this.requestOrganizationContact.organizationId = project.organizationId
          } else {
            console.warn('No projectId in UpdateCommand')
          }
        }
      } catch (error) {
        console.error(error)
      ...
      try {
        ...
        console.log('ProjectList loaded:', this.ProjectList)
        console.log('ProjectList length:', this.ProjectList.length)
        console.log('First project:', this.ProjectList[0])
        await this.loadOrganizationContactList()
      } catch (error) {
    ...
    async loadOrganizationContactList() {
      try {
        ...
        console.log('OrganizationContactList : ', this.OrganizationContactList)
      } catch (error) {
    ...
    async UpdateProjectContact() {
      ...
      if (valid) {
        try {
          this.loading = true
          console.log('UpdateCommand', this.UpdateCommand)
          const response = await client.updateProjectContact(this.UpdateCommand)
          if (response) {
            setTimeout(() => {
              this.loading = false
