    ...
    >
      <v-card class="page-container scroll-content dialog-scrollbar">
        <v-card-text>
          ...
          <v-form
            ref="form"
            @submit.prevent="UpdateOrganizationContact"
            class="text-black"
          >
...
export default defineComponent({
  name: 'UpdateOrganizationContactDetailView',
  props: {
    id: {
      type: String,
      required: true,
  ...
  methods: {
    ...
    async UpdateOrganizationContact() {
      ...
      if (valid) {
        try {
          this.loading = true
          const response = await client.updateOrganizationContact(this.UpdateCommand)
          if (response) {
            setTimeout(() => {
              this.loading = false
