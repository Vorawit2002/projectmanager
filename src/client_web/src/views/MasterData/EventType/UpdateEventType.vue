...
export default defineComponent({
  ...
  watch: {
    id() {
      this.initialize() // รีเฟรชข้อมูลเมื่อ id เปลี่ยน
    }
  },
  methods: {
    async initialize() {
    ...
    async UpdateEventType() {
      ...
      try {
        ...
        console.error(error)
        let errorMsg = 'เกิดข้อผิดพลาดในการแก้ไขข้อมูล ล้มเหลว!'
        if (this.isAxiosError(error)) {
          errorMsg = 'เกิดข้อผิดพลาด: ' + error.response.data.message
        }
        this.sweetAlertStore.error(errorMsg)
        this.closeDialog()
      } finally {
        this.loading = false
      }
    ...
    isAxiosError(err: unknown): err is { response: { data: { message: string } } } {
      return (
        typeof err === 'object' &&
        err !== null &&
        'response' in err &&
